...
namespace ShareData.Roles
{
    ...
    public class RoleAppService : AsyncCrudAppService<Role, RoleDto, int, PagedRoleResultRequestDto, CreateRoleDto, RoleDto>, IRoleAppService
    {
        ...
        public async Task<DatatableFilterdDto<RoleDto>> GetPaged(DatatableFilterInput input)
        {

            IQueryable<Role> query = Repository.GetAll().Where(e => !e.IsDeleted);

            int totalCount = await query.CountAsync();

            query = query
                .WhereIf(!string.IsNullOrEmpty(input.SearchTerm), b => b.Name.ToLower().Contains(input.SearchTerm) ||
                                                                         b.DisplayName.Contains(input.SearchTerm))
                ;
            ...
            return new DatatableFilterdDto<RoleDto>
            {
